...
## Steps
1. Read `ux-ui-agent-skills/workflows/governance.md` (SemVer table, contribution workflow, deprecation policy, change comms).
...
4. For a **deprecation**: mark with reason + replacement + removal version; keep working ≥ 1 minor cycle; provide a migration map (`ux-ui-agent-skills/design-systems/crosswalk.md` style); remove only in a major.
...
- `python3 ux-ui-agent-skills/scripts/validate_tokens.py` passes; contrast re-checked if colors changed.
