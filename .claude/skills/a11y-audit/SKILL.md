...
## Steps
1. Read `ux-ui-agent-skills/accessibility/wcag-checklist.md` (POUR-organized, P0/P1/P2) and `ux-ui-agent-skills/accessibility/aria-patterns.md`.
...
4. **Contrast — measure, don't eyeball.** For rendered HTML, RUN the real-render gates and report their actual output (CLAUDE.md → Verification Protocol): `node ux-ui-agent-skills/scripts/measure_render.mjs <file> [--dark]` (every text element) AND `node ux-ui-agent-skills/scripts/verify_states.mjs <file> [--dark]` (every interactive element in default/hover/focus — catches hover-state failures). For loose color pairs, `python3 ux-ui-agent-skills/scripts/contrast.py "<fg>" "<bg>"`. Never state a ratio you did not measure.
5. Check reduced-motion handling (`ux-ui-agent-skills/taste/motion-choreography.md`).
